...
stop_loss = None
take_profit = None
risk_percent = 0.02  # Risk 2% of the balance per trade
trade_log = []
trade_records = []

# Backtest loop with refined rules
for i in range(3, len(data)):
    atr_filter = data['ATR'][i] > 0.2  # Volatility Filter: Only trade if ATR > 0.2
    volume_filter = data['Volume'][i] > 0.5 * data['Avg_Volume'][i]  # Volume filter: Trade if volume > 50% of avg
        ...
        if position is None:
            ...
            if data['Close'][i] > data['EMA_50'][i] and data['Close'][i-1] < data['Close'][i-2] and data['Close'][i-1] < data['Close'][i-3]:
                if data['Close'][i] > max(data['High'][i-4:i]):
                    ...
                    trade_risk = min(balance * risk_percent, entry_price - stop_loss)  # Risk 2% of balance
                    take_profit = entry_price + 2 * (entry_price - stop_loss)
                    position = 'long'
                    balance -= trade_risk  # Deduct the risk from balance
                    trade_log.append({'action': 'buy', 'price': entry_price, 'stop_loss': stop_loss, 'take_profit': take_profit, 'balance': balance})
            ...
            # Check for short entry
                ...
                if data['Close'][i] < min(data['Low'][i-4:i]):
                    ...
                    position = 'short'
                    balance -= trade_risk  # Deduct the risk from balance
                    trade_log.append({'action': 'sell', 'price': entry_price, 'stop_loss': stop_loss, 'take_profit': take_profit, 'balance': balance})
                    trade_records.append([data['Time'][i], 'sell', entry_price, stop_loss, take_profit, balance])

    # If there is an open position, look for exit signals
    if position == 'long' and (data['Close'][i] <= stop_loss or data['Close'][i] >= take_profit):
        # Exit long position
        balance += (data['Close'][i] - entry_price)
        trade_log.append({'action': 'exit', 'price': data['Close'][i], 'balance': balance})
        trade_records.append([data['Time'][i], 'exit', data['Close'][i], balance])
        position = None
...
trade_df.to_csv('trade_log2.csv', index=False)
print("Trade log saved as 'trade_log.csv'")

# Analyze performance
print(f"Final Balance: ${balance}")
print(f"Total Trades: {len([log for log in trade_log if log['action'] == 'buy' or log['action'] == 'sell'])}")
